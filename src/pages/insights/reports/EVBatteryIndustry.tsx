...
const EVBatteryIndustry = () => {
  ...
  const content = {
    ...
    // NEW: Deep-Dive Analysis Section (HBR-style)
    deepDive: {
      title: {
...
Thailand has no lithium resources and minimal chemical processing capability. Unlike Indonesia (nickel) or Australia (lithium), Thailand cannot control upstream battery materials. This creates structural dependency that shapes all downstream investment decisions.`,
...
**中国との比較：**
中国のEV支配は、10年以上にわたる積極的な産業政策から生まれた：1,000億ドルを超える補助金、保護された国内市場、国家主導のバッテリーサプライチェーン統制。タイにはそのような介入を可能にする国内市場規模（7,000万人対14億人）も政治的能力もない。代わりに、タイは「中立的な製造プラットフォーム」として機能し、あらゆる出自からの投資を受け入れつつ、西側市場で中国OEMを制限する地政学的関与を回避している。

**EUとの比較：**
欧州の転換は規制主導型（2035年までのICE禁止）であり、確立されたプレミアムOEMが不本意ながら方向転換している。タイにはそのような規制圧力がない——30@30政策は拘束力があるというより理想的なものである。転換が起こっているのは、新規参入者（中国OEM）がタイを低リスクの輸出プラットフォームと見なしているからであり、既存プレイヤーが変化を強いられているからではない。
...
タイにはリチウム資源がなく、化学処理能力も最小限である。インドネシア（ニッケル）やオーストラリア（リチウム）とは異なり、タイは上流のバッテリー材料を制御できない。これにより、すべての下流投資決定を形作る構造的依存が生まれる。`
          }
        },
        boiTradeoffs: {
          title: {
...
While not legally binding, BOI approval implicitly expects: technology transfer to Thai suppliers within 3-5 years, employment of Thai nationals in management positions (typically 70%+ by year 5), and gradual increase in local content beyond minimum requirements. Companies that fail to meet these unstated expectations face difficulties in subsequent applications for additional activities or expansions.
...
**暗黙の期待：**
法的拘束力はないものの、BOI承認は暗黙のうちに以下を期待している：3〜5年以内のタイサプライヤーへの技術移転、タイ国籍者の経営職への登用（通常5年目までに70%以上）、最低要件を超えるローカルコンテンツの段階的増加。これらの暗黙の期待を満たさない企業は、追加活動や拡張の後続申請で困難に直面する。
...
**クローバック・リスク：**
指定された期間内に約束された投資額、雇用目標、輸出比率を達成できない場合、BOIインセンティブは取り消される可能性がある。歴史的に執行は寛大であったが、現政権は監査と遵守執行への意欲が高まっている——特に注目を集めるプロジェクトに対して。
...
**組立ではなくバッテリーが重要な理由：**
EV組立は基本的に低マージンのコモディティ的活動である。車両プラットフォーム、ソフトウェア、バッテリーが車両の価値と差別化の70〜80%を決定する。中国OEMは、組立事業が比較的控えめな資本要件（初期キャパシティに2〜4億ドル）で迅速に（12〜18ヶ月）確立できることを実証している。対照的に、バッテリーセル生産には10〜20億ドルの投資、3〜5年のリードタイム、容易に複製できない独自技術が必要である。
...
**日本の制約：**
日本のバッテリー技術（パナソニック、Envision AESC、トヨタの全固体開発）は、歴史的に異なる化学優先事項（コストのためのLFPではなく航続距離/性能のためのNMC）を持つハイエンドアプリケーションに焦点を当ててきた。これにより、タイの新興市場フォーカスとの技術ミスマッチが生じる。そこでは中国製LFPセルが20〜30%のコスト優位性により支配的である。
...
Japanese headquarters consistently underestimate the speed of market change in Thailand. Internal planning cycles of 3-5 years are misaligned with a market moving in 12-18 month cycles. Example: A major Japanese OEM's 2022 Thailand EV strategy assumed BEV market share reaching 10% by 2027. Actual 2024 data shows BEV already at 12% and accelerating. The strategy became obsolete before implementation began.

**Misread #2: Competitor Capability**
Japanese executives often describe Chinese competitors as "low quality" or "unsustainable." Field evidence contradicts this: Chinese EV models in Thailand consistently score high in J.D. Power quality surveys, and customer satisfaction for BYD's Atto 3 rivals Japanese premium brands. The competitive threat is real and growing, not a temporary market distortion.
...
**誤解#1：タイムライン圧縮**
日本本社は一貫してタイにおける市場変化のスピードを過小評価している。3〜5年の内部計画サイクルは、12〜18ヶ月サイクルで動く市場と整合していない。例：ある大手日系OEMの2022年タイEV戦略は、BEV市場シェアが2027年までに10%に達すると想定していた。2024年の実際のデータは、BEVがすでに12%に達し加速していることを示している。戦略は実施開始前に陳腐化した。

**誤解#2：競合他社の能力**
日本の経営幹部はしばしば中国競合他社を「低品質」または「持続不可能」と表現する。フィールドの証拠はこれに矛盾する：タイの中国製EVモデルはJ.D.Power品質調査で一貫して高スコアを獲得しており、BYDのAtto 3に対する顧客満足度は日本のプレミアムブランドに匹敵する。競争上の脅威は現実であり拡大している——一時的な市場の歪みではない。
...
- Chinese investment momentum slows but doesn't reverse
...
*Current status:* Chinese applications comprise approximately 65-70% of active pipeline (as of Q4 2024)
...
*Current status:* EV technical roles showing 15-25% annual salary increases; general automotive roles at 5-8%
...
*現状：* 2024年第4四半期時点で、中国申請がアクティブパイプラインの約65〜70%を占める
...
*現状：* EV技術職は年間15〜25%の給与増加；一般自動車職は5〜8%
    ...
    <>
      ...
      <div className="min-h-screen bg-background">
        ...
        <main className="container mx-auto px-4 py-8">
          ...
          {/* NEW: Deep-Dive Analysis Section */}
          {hasFullAccess ? (
            <section className="mb-12">
              <div className="mb-8">
                ...
                <p className="text-muted-foreground">
                  {isJapanese 
                    ? "以下は、タイEV産業の構造的変革についてのハーバード・ビジネス・レビュー風の詳細分析です。概要のみをお読みになるか、各セクションを深く掘り下げてお読みください。"
                    : "The following is an HBR-style deep-dive analysis on Thailand's EV industry structural transformation. Read the overview or deep-dive into each section."}
                </p>
              </div>
